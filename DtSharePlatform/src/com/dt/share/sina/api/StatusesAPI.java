...
public class StatusesAPI extends WeiboAPI
{
     ...
     * @param listener
     */
    public void mentions(long since_id, long max_id, int count, int page, AUTHOR_FILTER filter_by_author, SRC_FILTER filter_by_source, TYPE_FILTER filter_by_type, boolean trim_user, RequestListener listener)
    {
        WeiboParameters params = new WeiboParameters();
        params.add("since_id", since_id);
        params.add("max_id", max_id);
     ...
     *
     * @param content  要发布的微博文本内容，内容不超过140个汉字
     * @param filePath     要上传的图片，仅支持JPEG、GIF、PNG格式，图片大小小于5M。
     * @param listener
     */
    public void uploadPic(String content, String filePath, RequestListener listener)
    {
        WeiboParameters params = new WeiboParameters();
        params.add("status", content);
